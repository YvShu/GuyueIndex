...
for _, config in enumerate(group_config):
    ...
    for match in matches:
        ...
        if config == "sift_clustered2_DeDrift" or config == "sift_clustered2_Ada" or config == "sift_clustered2_LIRE" or config == "sift_clustered2_None":
            time_val *= 1000
...
ax.set_ylabel("Queries per Second", fontsize=15)
...
# 保存为高质量科研图像
plt.savefig(f'/mnt/hgfs/DataSet/experiment_results/{workload}-LVQ_{dataset}_qps_q{q}.svg', dpi=150, bbox_inches='tight')
plt.show()
