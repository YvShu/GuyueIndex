 ...
 * @LastEditTime: 2026-03-24 17:09:42
 * @LastEditors: Guyue
...
int main()
{
    ...
    std::string vectors_file_path = "/mnt/hgfs/DataSet/" + dataset + "/workload1/" + dataset_name + "_base.fvecs";
    std::string queries_file_path = "/mnt/hgfs/DataSet/" + dataset + "/" + dataset_name + "_query.fvecs";
    std::string gt_dir_path = "/mnt/hgfs/DataSet/" + dataset + "/workload1/";
    std::string runbook_file_path = "/mnt/hgfs/DataSet/" + dataset + "/workload1/runbook.json";
    const std::string output_csv_path = "../output/workload1/" + dataset + "_Tree-LIRE_" + std::to_string(k) + ".csv";
    ...
    reindexing_params->reindexing_radius = 5;
    reindexing_params->refinement_iterations = 5;
    reindexing_params->max_partition_size = 550;
    reindexing_params->min_partition_size = 50;
    ...
    for (size_t op_i = 0; op_i < operations.size(); ++op_i)
    {
        ...
        else if (op_type == "search")
        {
            ...
            while (search_params->nprobe < index->nlist())
            {
                ...
                if (std::abs(recalls - target_recall) <= 0.005 || nprobe == 1)
                {
                    search_results = index->search(n_queries, queries, search_params);
        ...
        else if (op_type == "delete")
        {
            ...
            double reindexing_time = 0.0f;
            for (size_t b = 0; b < div_roundup(delete_size, batch_size_); ++b)
            {
                size_t begin = b * batch_size_;
                size_t bs = std::min(delete_size - begin, batch_size_);
                std::vector<int64_t> batch_ids_(bs);
                for (size_t i = 0; i < bs; ++i)
                {
                    batch_ids_[i] = i + begin + start;
